...
func buildBIOSSettings(d *schema.ResourceData, firmwareConfig *bmc.FirmwareConfig) (settings []map[string]string, err error) {
	...
	// disableCertificateVerification is default omitempty, if not specifying it as true in the install-config
	// it would not exits in driver_info
	var disableCertificateVerification bool
	driverInfo := d.Get("driver_info").(map[string]interface{})
	if _, ok := driverInfo["irmc_verify_ca"]; ok {
		disableCertificateVerification = true
	} else {
		disableCertificateVerification = false
	}

	acc, err := bmc.NewAccessDetails(d.Get("bmc_address").(string), disableCertificateVerification)
	if err != nil {
		return nil, err
	}
